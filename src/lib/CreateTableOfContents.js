...
import { getPagePosterities, getHeadingLevel, getHeading, isTopLayer } from "./Utilities"
import { TocPageName, ArtboardSortEnum, ArtbaordLayersSortEnum } from "./Constants"
...
function createHeading(artboard, serialIdx, pageIdx, layer) {
  ...
  let text = ""
  let combinatorWidth = 800 - serialAndContent.frame.x - serialAndContent.frame.width
...
function checkArtboardInnerLayersSort(arboardInnerLayersSort) {
  if (arboardInnerLayersSort === "Top to Bottom") {
    return ArtbaordLayersSortEnum.Top2BottomByLayerList
  } else if (arboardInnerLayersSort === "Bottom to Top") {
    return ArtbaordLayersSortEnum.Top2BottomByLayerList
  }
}

export default function(artboardSort, artboardInnerLayersSort) {
  // get layers
  let pages = document.pages
  let layers = []
  for (let page of pages) {
    if (page.isSymbolsPage() || page.name === TocPageName) continue
    let list = getPagePosterities(
      page,
      checkArtboardSort(artboardSort),
      checkArtboardInnerLayersSort(artboardInnerLayersSort)
    )
